...
# run this script in MSYS2 shell (MSYS2 MINGW64)
# in MSYS2
# pacman -S perl-Locale-Maketext-Simple

ARCH="${1:-}"
if [ "$ARCH" != "arm64" ] && [ "$ARCH" != "x86_64" ] && [ "$ARCH" != "x86" ]; then
	echo "Usage: $0 {arm64|x86_64|x86}"
	exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
OUTPUT_DIR="$SCRIPT_DIR/openssl-win32-$ARCH"
